...
public class InOrOutListBasedFilterTest {
	
	static private final Logger LOG = LoggerFactory.getLogger(InOrOutListBasedFilterTest.class);
	...
	public void canSeeFileChanges() throws Exception {
		...
		InOrOutListBasedFilter underTest = createFilterBasedOnList(inFile, true, 1);
	...
	public void canLoadBigFile() throws Exception {
		...
		InOrOutListBasedFilter underTest = createFilterBasedOnList(inFile, true, 10000);
	...
	public void canFilterDT() throws Exception {
		InOrOutListBasedFilter underTest = createFilterBasedOnList("test-refdata/imsi.txt", false, 10000);
	...
	private InOrOutListBasedFilter createFilterBasedOnList(String inFile, boolean inMode, int refreshFrequency) throws Exception {
		InOrOutListBasedFilter filter = new InOrOutListBasedFilter();
		filter.setInMode(inMode);
		filter.setInOrOutFilePath(inFile);
		filter.setRefresh(refreshFrequency);
		filter.loadListFromFileAndStartWatcher();
		return filter;
	}
}
