 ...
 */
public class MiniMapJobExecutionDao implements JobExecutionDao {
	
	private static final Logger LOG = LoggerFactory.getLogger(MiniMapJobExecutionDao.class);
	...
	private MiniMapDeleteListener deleteListener;

	public MiniMapJobExecutionDao(MiniMapDeleteListener deleteListener) {
	...
	public void updateJobExecution(JobExecution jobExecution) {
		...
		synchronized (jobExecution) {
			...
			if(status.equals(BatchStatus.COMPLETED) || status.isGreaterThan(BatchStatus.STOPPING)) {
				LOG.debug("Deleting {} of status {}", jobExecution.getId(), jobExecution.getStatus());
				executionsById.remove(id);
				deleteListener.delete(jobExecution);
				LOG.debug("Deleting {} of status {} map size is {}", jobExecution.getId(), jobExecution.getStatus(), executionsById.size());
			} else {
				LOG.debug("Updating {} of status {}", jobExecution.getId(), jobExecution.getStatus());
